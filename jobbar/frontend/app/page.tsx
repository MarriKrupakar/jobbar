...
export default function LandingPage() {
  ...
  useEffect(() => {
    // Handle Supabase OAuth callback — token comes back in URL hash
    const handleCallback = async () => {
      ...
      // Also check URL params (PKCE flow)
  ...
  if (loading) return (
    <div style={{display:'flex',alignItems:'center',justifyContent:'center',minHeight:'100vh',background:'var(--bg-primary)'}}>
      <div style={{textAlign:'center'}}>
        <div style={{width:40,height:40,border:'3px solid var(--accent)',borderTopColor:'transparent',borderRadius:'50%',margin:'0 auto 16px',animation:'spin 0.8s linear infinite'}}/>
  ...
  return (
    <div style={{minHeight:'100vh',background:'var(--bg-primary)',display:'flex',alignItems:'center',justifyContent:'center',position:'relative',overflow:'hidden'}} className="grid-bg">
      ...
      <motion.div initial={{opacity:0,y:30}} animate={{opacity:1,y:0}} transition={{duration:0.6}}
        style={{width:'100%',maxWidth:440,margin:'0 auto',padding:'0 20px',position:'relative',zIndex:10}}>

        {/* Logo */}
        <div style={{textAlign:'center',marginBottom:32}}>
          <div style={{display:'inline-flex',alignItems:'center',gap:10,marginBottom:8}}>
            <span style={{fontFamily:'Space Grotesk,sans-serif',fontSize:32,fontWeight:800,color:'var(--text-primary)',letterSpacing:'-0.02em'}}>
              job<span style={{color:'var(--accent)'}}>.bar</span>
            </span>
          </div>
        ...
        <div className="glass-card" style={{padding:32}}>
          ...
          {/* Google Button */}
          <motion.button onClick={signInWithGoogle} whileHover={{scale:1.02}} whileTap={{scale:0.98}}
            style={{width:'100%',display:'flex',alignItems:'center',justifyContent:'center',gap:12,padding:'13px 20px',
              background:'rgba(255,255,255,0.08)',border:'1px solid rgba(255,255,255,0.15)',borderRadius:12,
              color:'var(--text-primary)',fontSize:15,fontWeight:600,cursor:'pointer',fontFamily:'Inter,sans-serif',marginBottom:24}}>
          ...
          <p style={{textAlign:'center',marginTop:16,fontSize:13,color:'var(--text-muted)'}}>
            No account?{' '}
            <span onClick={signInWithGoogle} style={{color:'var(--accent)',cursor:'pointer'}}>Sign up free</span>
          </p>
        </div>

        <p style={{textAlign:'center',marginTop:20,fontSize:12,color:'var(--text-muted)'}}>
          🔒 Trusted by 50,000+ job seekers · No spam · Cancel anytime
        </p>
      </motion.div>
    </div>
  );
